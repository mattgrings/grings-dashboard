    ...
    return (
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="space-y-4">
        <div className="bg-[#00E620]/10 border border-[#00E620]/20 rounded-xl p-4 text-center space-y-2">
          ...
          <p className="text-gray-400 text-sm">Verifique sua caixa de entrada para redefinir a senha.</p>
        </div>
        <button onClick={onVoltar} className="w-full text-sm text-gray-500 hover:text-white transition-colors flex items-center justify-center gap-2">
    ...
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="space-y-1.5">
          ...
          <div className={`flex items-center gap-2 border rounded-xl px-3 transition-all ${email ? 'border-[#00E620] shadow-[0_0_12px_rgba(0,230,32,0.15)]' : 'border-white/10'}`}>
            <EnvelopeSimple size={18} className={email ? 'text-[#00E620]' : 'text-gray-600'} />
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="seu@email.com" autoComplete="email" className="flex-1 bg-transparent py-3.5 text-white placeholder:text-gray-600 outline-none text-base" />
          </div>
        </div>

        {erro && (
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-2 text-red-400 text-sm bg-red-500/10 border border-red-500/20 rounded-xl px-3 py-2.5">
            <Warning size={16} weight="fill" /> {erro}
          </motion.div>
        )}

        <motion.button type="submit" disabled={loading} whileTap={{ scale: 0.97 }} className="w-full py-3.5 rounded-xl bg-gradient-to-r from-[#00E620] to-[#00CC00] text-black font-bold text-sm tracking-wide transition-all disabled:opacity-50 touch-manipulation">
          {loading ? 'Enviando...' : 'Solicitar redefinição'}
        </motion.button>

        <button type="button" onClick={onVoltar} className="w-full text-sm text-gray-500 hover:text-white transition-colors flex items-center justify-center gap-2">
          <ArrowLeft size={16} /> Voltar ao login
        </button>
      </form>
    </motion.div>
...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setErro('')
    try {
      const perfil = await loginFn(email.trim(), senha)
            ...
            {modo === 'login' ? (
              <motion.div key="login" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                <div className="text-center mb-6">
                  ...
                  <p className="text-gray-500 text-sm mt-1">Entre com seu email e senha</p>
                </div>
                ...
                <form onSubmit={handleLogin} className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className={`flex items-center gap-2 border rounded-xl px-3 transition-all ${senha ? 'border-[#00E620] shadow-[0_0_12px_rgba(0,230,32,0.15)]' : 'border-white/10'}`}>
                      ...
                      <input
                        ...
                        className="flex-1 bg-transparent py-3.5 text-white placeholder:text-gray-600 outline-none text-base"
                      />
                      <button type="button" onClick={() => setMostrarSenha(!mostrarSenha)} className="touch-manipulation p-1">
                        {mostrarSenha
                ...
                <button
                  ...
                  className="w-full text-center text-sm text-gray-500 hover:text-[#00E620] transition-colors mt-4 touch-manipulation py-2"
                >
                  Esqueci minha senha
                </button>
              </motion.div>
