...
import AbaCaptacao from '../components/crm/AbaCaptacao'
...
export interface CaptacaoVolume {
  id: string
  data: string
  quantidade: number
  canal: string
...
type AbaCRM = 'dashboard' | 'captacao' | 'pipeline' | 'followup' | 'alertas'
...
export default function CRMPage() {
  ...
  const [captacoesVolume, setCaptacoesVolume] = useState<CaptacaoVolume[]>([])
  ...
  const carregar = useCallback(async () => {
    ...
    const [{ data: leadsData }, { data: volumeData }, { data: alunosData }, { data: metaData }] =
      await Promise.all([
        supabase.from('captacoes').select('*').order('criado_em', { ascending: false }),
        supabase.from('captacoes_volume').select('*').order('data', { ascending: false }).limit(60),
        supabase
          .from('perfis')
    ...
    setCaptacoesVolume((volumeData as CaptacaoVolume[]) ?? [])
  ...
  const abas: { id: AbaCRM; label: string; emoji: string; badge: number | null }[] = [
    ...
    { id: 'captacao', label: 'Captação', emoji: '📥', badge: null },
    {
      id: 'pipeline',
      label: 'Pipeline',
      emoji: '🔄',
      ...
      <div className="flex gap-2 overflow-x-auto scrollbar-none -mx-4 px-4">
        {abas.map((tab) => (
          ...
          >
            ...
            {tab.label}
            {tab.badge ? (
              <span className="min-w-[20px] h-5 rounded-full text-xs font-bold bg-red-500 text-white flex items-center justify-center px-1">
                {tab.badge}
              </span>
            ) : null}
          </button>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {aba === 'dashboard' && (
              <DashboardCRM
                leads={leads}
                captacoesVolume={captacoesVolume}
                alertas={alertas}
                metas={metas}
                mesAtual={mesAtual}
